...
export class BudgetingService {
  ...
  async getBudgetStatus(familyId: string, period: string) {
    ...
    const result = budgets.map(b => ({
      ...
      remaining: Math.max(0, Number(b.amount) - (spendingByCategory[b.category] || 0)),
      percentage: Math.min(100, ((spendingByCategory[b.category] || 0) / Number(b.amount)) * 100),
    }));
   ...
   */
  async setBudget(familyId: string, category: string, amount: number, period: string) {
